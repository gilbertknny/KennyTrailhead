...
export default class LwcInquiryReportBriva extends LightningElement {
    ...
    handleCorpCode(event){
        ...
        // }
        // console.log('search button' , this.disableSearchButton);
        
        // event.target.reportValidity();

        this.corpCode = event.target.value;
    }

    handleCustCode(event){
        this.custCode = event.target.value;
    }
            ...
            if(result){
                ...
                if(responseData){
                    ...
                    this.data = responseData.map((item, index) => {
                        return {
                            ...item,
                            fullBriva:item.brivaNo+item.custCode,
                            No: this.getIndex(index)
                        }
                    });

                    console.log('full briva',this.data[0].fullBriva );
