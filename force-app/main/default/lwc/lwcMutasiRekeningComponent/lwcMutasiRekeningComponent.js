...
export default class LwcMutasiRekeningComponent extends NavigationMixin(LightningElement) {
    ...
    handleCheckboxChange(event) {
        ...
        console.log(`Checkbox clicked with ID: ${selectedId} from source: ${source}, checked: ${isChecked}`);
    
        // Clear the selection in both data sources first
        this.latestTransactions = this.latestTransactions.map(trans => ({
            ...trans,
            isSelected: false
        }));
        ...
        // Then set the selected item based on source
        if (source === 'latest') {
            this.latestTransactions = this.latestTransactions.map(trans => ({
                ...trans,
                isSelected: trans.id === selectedId && isChecked
            }));
        } else {
            this.filteredData = this.filteredData.map(row => ({
                ...row,
                isSelected: row.id === selectedId && isChecked
            }));
        }
    
        // Update selectedRow based on check status
        this.selectedRow = isChecked ? selectedId : null;
    
        // Find the selected item from the appropriate data source
        const selectedLatestTransaction = this.latestTransactions.find(row => row.isSelected);
        const selectedSearchResult = this.filteredData.find(row => row.isSelected);
        
        // Set the checkedData based on which data source has a selected item
        this.checkedData = selectedLatestTransaction || selectedSearchResult || null;
    
        // Only get TID if we have selected data
        if (this.checkedData) {
            const savingMutationData = source === 'latest' ? 
                this.prepareLatestTransactionData(this.checkedData) :
                this.prepareRegularTransactionData(this.checkedData);
            ...
            // Clear TID when nothing is selected
            this.tidValue = null;
        }
    ...
    prepareLatestTransactionData(transaction) {
        // Ensure jamTran is properly formatted
        let jamTran = transaction.customTime || '';
        jamTran = jamTran.replace(/:/g, '');
        
        // Clean the mutasiDebet value by removing currency formatting
        let mutasiDebet = 0;
        if (transaction.mutasiDebet) {
            const rawDebet = String(transaction.mutasiDebet).replace(/[^\d,-]/g, '').replace(/\./g, '').replace(',', '.');
            mutasiDebet = parseFloat(rawDebet) || 0;
        }
        ...
        return {
            ...
            idNum3: 0,
            seq: 0,
            noRek: transaction.noRek ? transaction.noRek.toString().replace(/\D/g, '') : '0',
            tglTran: transaction.customDate || '',
            tglEfektif: transaction.customDate || '',
            jamTran: jamTran || 0,
            kodeTran: '',
    ...
    prepareRegularTransactionData(transaction) {
        // Clean mutasiDebet and mutasiKredit values
        let mutasiDebet = 0;
        let mutasiKredit = 0;
        ...
        if (transaction.mutasiKredit) {
            const rawKredit = String(transaction.mutasiKredit).replace(/[^\d,-]/g, '').replace(/\./g, '').replace(',', '.');
            mutasiKredit = parseFloat(rawKredit) || 0;
        }
        
        // Clean jamTran if needed
        let jamTran = transaction.jamTran || 0;
        if (typeof jamTran === 'string' && jamTran.includes(':')) {
            jamTran = jamTran.replace(/:/g, '');
        }
        ...
        return {
            ...
            seq: transaction.seq || 0,
            noRek: transaction.noRek ? transaction.noRek.toString().replace(/\D/g, '') : '0',
            tglTran: transaction.tglTran || transaction.customDate || '',
            tglEfektif: transaction.tglEfektif || transaction.customDate || '',
            jamTran: jamTran,
            kodeTran: transaction.kodeTran || '',
            deskTran: transaction.deskTran || transaction.customRemark || '',
            saldoAwalMutasi: parseFloat(transaction.saldoAwalMutasi) || 0,
            mutasiDebet: mutasiDebet,
            mutasiKredit: mutasiKredit,
            saldoAkhirMutasi: parseFloat(transaction.saldoAkhirMutasi) || 0,
            truser: transaction.truser || '',
            glsign: transaction.glsign || '',
            terbilang: transaction.terbilang || '',
            trremk: transaction.trremk || '',
    ...
    handleCreateCase() {
        ...
        // Look for selected data in both data sources
        const selectedLatestTransaction = this.latestTransactions.find(row => row.isSelected);
        const selectedSearchResult = this.filteredData.find(row => row.isSelected);
        
        // Use whichever data source has the selected item
        const selectedData = selectedLatestTransaction || selectedSearchResult;
    
        if (selectedData) {
            console.log('Selected Data for Create Case:', JSON.stringify(selectedData));
            const fields = {};
    
            // Ensure RecordTypeId is valid
            if (!this.recordTypeId) {
                console.error('Record Type ID is missing');
                this.showToast('Error', 'Record Type ID is missing. Please contact your administrator.', 'error');
                this.isLoadingCreateCase = false;
                return;
            }
            fields['RecordTypeId'] = this.recordTypeId;
    
            // Format date field - ensure we have a valid date
            let formattedDate;
            try {
                // Handle different date formats that might come from the UI
                const dateValue = selectedData.customDate;
                ...
                this.showToast('Error', 'Invalid transaction date format', 'error');
                this.isLoadingCreateCase = false;
                return;
            }
            ...
            if (formattedDate && selectedData.customTime) {
                try {
                    const timeString = selectedData.customTime.trim();
                    let hours, minutes, seconds;
                    
                    if (timeString.includes(':')) {
                        [hours, minutes, seconds = '00'] = timeString.split(':');
                    } else if (timeString.length === 6) {
                        // Format like "173705"
                        hours = timeString.substring(0, 2);
                        minutes = timeString.substring(2, 4);
                        seconds = timeString.substring(4, 6);
                    } else {
                        throw new Error('Unrecognized time format');
                    }
                    
                    // Ensure values are padded properly
                    const paddedTime = `${hours.padStart(2, '0')}:${minutes.padStart(2, '0')}:${seconds.padStart(2, '0')}`;
                    const combinedDateTime = `${formattedDate}T${paddedTime}`;
                    const dateTimeObject = new Date(combinedDateTime);
                    ...
                    fields['SCC_Waktu_Transaksi__c'] = dateTimeObject.toISOString();
                    console.log('Formatted datetime:', fields['SCC_Waktu_Transaksi__c']);
                } catch (error) {
                    console.error('Time parsing error:', error);
                    this.showToast('Error', 'Invalid transaction time format', 'error');
                    this.isLoadingCreateCase = false;
                    return;
                }
            }
    
            // Handle amount based on transaction type - parse carefully
            let amount = 0;
            try {
                if (selectedLatestTransaction) {
                    // For latest transactions - first try direct value if available
                    if (typeof selectedData.Amt === 'number') {
                        amount = selectedData.Amt;
                    } else {
                        // Fall back to formatted string parsing
                        const rawValue = String(selectedData.mutasiDebet || '0');
                        // First replace dots (thousand separators in ID locale), then replace comma with dot for decimal point
                        const cleanedValue = rawValue.replace(/\./g, '').replace(/,/g, '.');
                        // Remove any remaining non-numeric chars except decimal point and negative sign
                        const numericValue = cleanedValue.replace(/[^\d.-]/g, '');
                        amount = parseFloat(numericValue) || 0;
                    }
                } else {
                    // For search results
                    let debitValue = 0;
                    ...
                    // Process debit value if present
                    if (selectedData.mutasiDebet) {
                        const rawDebit = String(selectedData.mutasiDebet || '0');
                        const cleanedDebit = rawDebit.replace(/\./g, '').replace(/,/g, '.').replace(/[^\d.-]/g, '');
                        debitValue = parseFloat(cleanedDebit) || 0;
                    }
                    
                    // Process credit value if present
                    if (selectedData.mutasiKredit) {
                        const rawCredit = String(selectedData.mutasiKredit || '0');
                        const cleanedCredit = rawCredit.replace(/\./g, '').replace(/,/g, '.').replace(/[^\d.-]/g, '');
                        creditValue = parseFloat(cleanedCredit) || 0;
                    }
                    
                    // Choose the non-zero value, preferring debit if both are present
                    if (debitValue !== 0) {
                        amount = debitValue;
                ...
                this.showToast('Error', 'Invalid amount format', 'error');
                this.isLoadingCreateCase = false;
                return;
            }
            
            fields['SCC_Amount__c'] = amount;
            
            // Handle account number
            if (selectedData.noRek) {
                // Ensure it's a string and remove any non-numeric characters
                const accountNumber = String(selectedData.noRek).replace(/\D/g, '');
                fields['SCC_Account_Number__c'] = accountNumber;
                console.log('Account Number:', accountNumber);
            } else {
                console.warn('No account number found in selected data');
            }
            
            // Handle card number if available
            if (this.noKartu) {
                fields['SCC_Card_Number__c'] = String(this.noKartu).trim();
            }
            
            // Handle terminal ID if available
            if (this.tidValue) {
                fields['SCC_Terminal_ID__c'] = String(this.tidValue).trim();
            }
            
            // Ensure we have a valid AccountId
            if (!this.accountId) {
                console.error('Account ID is missing');
                this.showToast('Error', 'Account ID is missing. Please contact your administrator.', 'error');
                this.isLoadingCreateCase = false;
                return;
            }
            fields['AccountId'] = this.accountId;
    
            // Log the complete fields object for debugging
            console.log('Fields for Case creation:', JSON.stringify(fields));
    
            const recordInput = { apiName: 'Case', fields };
    
            createRecord(recordInput)
                .then(caseRecord => {
                    console.log('Successfully created record:', JSON.stringify(caseRecord));
                    this.showToast(
                        'Sukses',
                        'Record berhasil ditambahkan! anda akan diarahkan ke halaman Case. Atau ',
                        'success',
                ...
                .catch(error => {
                    ...
                    console.error('Error creating case:', JSON.stringify(error, null, 2));
                    ...
                    if (error.body) {
                        if (error.body.message) {
                            ...
                            const fieldErrors = error.body.output.errors
    ...
    handleUpdateCase() {
        this.isLoadingUpdateCase = true;
        console.log('Starting case update process...');
        ...
        if (selectedData) {
            console.log('Selected Data for Update Case:', JSON.stringify(selectedData));
            const fields = {};
            
            // Ensure we have a valid record ID
            if (!this.recordId) {
                console.error('Record ID is missing');
                this.showToast('Error', 'Record ID is missing. Cannot update case.', 'error');
                this.isLoadingUpdateCase = false;
                return;
            }
            fields['Id'] = this.recordId;
            ...
            // Include AccountId if available
            if (this.accountId) {
                fields['AccountId'] = this.accountId;
            }
    
            // Log the complete fields object for debugging
            console.log('Fields for Case update:', JSON.stringify(fields));
    
            const recordInput = { fields };
    
            updateRecord(recordInput)
                .then(caseRecord => {
                    console.log('Successfully updated record:', JSON.stringify(caseRecord));
                    this.showToast(
                        'Sukses',
                        'Record berhasil diperbarui. ',
                        'success',
                ...
                .catch(error => {
                    // Enhanced error logging
                    console.error('Error updating case:', JSON.stringify(error, null, 2));
                    let errorMessage = 'An unknown error occurred.';
