...
import getMyCampaignDetails from '@salesforce/apex/SCC_CampaignDetailAssignmentController.getMyCampaignDetails';
...
export default class CampaignDetailAssignment extends NavigationMixin(LightningElement) {
    ...
    async loadMyCampaignDetails() {
        this.showSpinner = true;
        try {
            const result = await getMyCampaignDetails();
            ...
            this.showToast('Error', 'Error loading my campaign details: ' + error.message, 'error');
        } finally {
            this.showSpinner = false;
        }
    }
    ...
    handleAssign() {
        // Initial validation for empty selections
        ...
        // New validation: If only one campaign detail is selected, limit maker selection to one
        if (this.selectedRowIds.length === 1 && this.selectedMakerIds.length > 1) {
            this.showToast(
                'Error', 
                'When selecting a single campaign detail, you can only assign it to one maker', 
                'error'
            );
            return;
        }
