...
export default class AddNewCoverageRate extends LightningElement {
        ...
        if (!this.searchTerm && this.selectedCoverageId == '0') {
            if(this.bsnId == '301'){
                return this.coverageDataFormated.filter(row =>
                    row.parentCoverageId == '0' && row.sectionDisplay == 1
                );
            }else{
                return this.coverageDataFormated.filter(row =>
                    row.sectionDisplay == 1
                );
            }
        }
        ...
        if(this.bsnId == '301'){
            if (this.selectedCoverageId) {
                ...
                dataToFilter = dataToFilter.filter(row => {
                    ...
                    return (isSelectedRow || isChildOfSelected) && row.sectionDisplay == 1;
                });
            }
            if (lowerCaseSearch) {
                dataToFilter = dataToFilter.filter(row =>
                    row.coverageName && row.coverageName.toLowerCase().includes(lowerCaseSearch) && row.sectionDisplay == 1
                );
            }
            return dataToFilter;
        }else{
    ...
    async getCoverageData(){
        try {
            return await getExistingCoverage({riskId : this.riskId,tahun: this.buttonClickedValue});
        }
        catch (error) {
    ...
    wiredCoverages({ error, data }) {
        if (data) {
            ...
            let dynamicRows = data.map((item, index) => ({
                ...
                coverageIdSection: item.coverageIdSection,
            ...
            if (this.jsonString) {
                ...
                try {
                    ...
                    const keyInclude = [
                        'batasBawah', 'coverageId', 'coverageIdSection', 
                        'coverageName', 'parentCoverageId', 'sectionDisplay', 
                        'sectionId', 'sectionName', 'id','riskId','policyId','rowType'
                    ];
                    parsedData.forEach(newItem => {
                        this.descriptionRate = newItem.rate.descriptionRate;
                        const targetId = newItem.id;
                        const targetSec = newItem.coverageIdSection;
                        // console.log('✅ CoverageRate targetId',targetId);
                        const targetItem = currentDataList.find(
                            item => item.id === targetId && item.coverageIdSection === targetSec
                        );
                        // console.log('✅ CoverageRate targetItem',targetItem);
                        if (targetItem) {
                            for (const key in newItem) {
                                if (newItem.hasOwnProperty(key)) {
                                    if (!keyInclude.includes(key)) {
                ...
                .then(existingResult => {
                    if (existingResult && existingResult.length > 0) {
                        ...
                        existingResult.forEach(existingItem => {
                            const targetId = existingItem.coverageIdSection;
                            ...
                            if (existingItem.coverageName.includes('+') || selectedIds.length > 1) {
                                ...
                                const targetItem = currentDataList.find(item => item.coverageIdSection === targetId);
                                if (targetItem) {
                                    const rateValue = existingItem.rate;
                                    console.log('breakdown rateValue',rateValue);
                                    const deductibleValue = existingItem.deductible;
                                    console.log('breakdown rateValue Deduct',deductibleValue);
                                    const rebateValue = existingItem.rebate;
                                    console.log('breakdown rateValue rebate',rebateValue);
                                    let calculatedPremium1 =0;
    ...
    applyDisableLogic(event) {
        ...
        this.coverageData = this.coverageData.map(item => {
            ...
            return {
                ...
                deductible: item.deductible ? { ...item.deductible } : {
                    deductibleSetting: ''
                },
