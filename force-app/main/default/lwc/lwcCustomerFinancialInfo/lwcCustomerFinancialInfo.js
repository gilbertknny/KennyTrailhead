...
export default class LwcCustomerFinancialInfo extends LightningElement {
    @api recordId
    @track accountId;
    ...
    //     },
    //     simpanan: [
    //         { rownum: 1, product: 'SiMuda', accountNumber: '0001223209218231', status: 'Aktif' },
    //         { rownum: 2, product: 'BritAma', accountNumber: '0001223209218232', status: 'Aktif' },
    //         { rownum: 3, product: 'BritAma X', accountNumber: '0001223209218233', status: 'Tidak Aktif' }
    //     ],
    //     pinjaman: [
    //         { rownum: 1, product: 'Kupedes', accountNumber: '0001223209218231' },
    //         { rownum: 2, product: 'KUR', accountNumber: '0001223209218232' },
    //         { rownum: 3, product: 'BRIGuna', accountNumber: '0001223209218233' }
    //     ],
    //     investasi: [
    //         { rownum: 1, product: 'DPLK', accountNumber: '05542274747390', status: 'Aktif' },
    //         { rownum: 2, product: 'BRI Life', accountNumber: '05542274747391', status: 'Aktif' },
    //         { rownum: 3, product: 'Reksadana', accountNumber: '05542274747392', status: 'Tidak Aktif' }
    //     ],
    //     cardlink: [
    //         { rownum: 1, product: 'BRI Touch', cardNumber: '0001223209218231', status: 'Aktif' },
    //         { rownum: 2, product: 'BRI Platinum', cardNumber: '0001223209218232', status: 'Aktif' },
    //         { rownum: 3, product: 'BRI JCB', cardNumber: '0001223209218233', status: 'Tidak Aktif' }
    //     ]
    // };
    ...
    //     { no: 5, produk: 'SiMuda', nomorRekening: '777888999', status: 'Aktif', brimo: 'Nonaktif' }
    // ];
    ...
    */
    ...
    get processedSimpananData() {
        // return this.simpananData.map(item => ({
        //     ...item,
        //     customStatus: item.status === 'Aktif' ? 'status-aktif' : 'status-nonaktif',
        //     customBrimo: item.brimo === 'Aktif' ? 'status-aktif' : 'status-nonaktif'
        // }));
        return this.simpananData.map((result, index) => {
            const simpananArray = result?.data?.portofolioPerbankan?.simpanan || [];
    
            // Map through the simpanan array and process each simpanan
            const processedSimpanan = simpananArray.map(simpanan => ({
                ...simpanan,
                customStatus: simpanan.status === 'Active' ? 'status-aktif' : 'status-nonaktif',
                customBrimo: simpanan.brimo === 'Active' ? 'status-aktif' : 'status-nonaktif'
            }));
    ...
    */
    ...
    //         const processedSimpanan = simpananArray.map(simpanan => ({
    //             ...simpanan,
    //             status: simpanan.status === '1' ? 'Aktif' : 'Non-Aktif',
    //             customStatus: simpanan.status === '1' ? 'status-aktif' : 'status-nonaktif',
    //             brimo: simpanan.brimo === '1' ? 'Aktif' : 'Non-Aktif',
    ...
    get processedKreditData() {
        return this.kreditData.map(item => ({
            ...item,
            customStatus: item.status === 'Aktif' ? 'status-aktif' : 'status-nonaktif',
        }));
    }
    */
    ...
    // get processedKreditData() {
    //     return this.kreditData.map((result, index) => {
    //         // const simpananArray = result?.data?.portofolioPerbankan?.simpanan || [];
    ...
    //             ...cardlink,
    //             status: cardlink.status === '1' ? 'Aktif' : 'Non-Aktif',
    //             customStatus: cardlink.status === '1' ? 'status-aktif' : 'status-nonaktif',
    //             // customBrimo: cardlink.brimo === '1' ? 'status-aktif' : 'status-nonaktif'
    ...
    //             ...result,
    //             no: index + 1,
    //             cardlink: processedCardlink // Include the processed simpanan array
    //         };
    //     });
    // }
    ...
    get processedPinjamanData() {
        return this.pinjamanData.map(item => ({
            ...item,
            customStatus: item.status === 'Aktif' ? 'status-aktif' : 'status-nonaktif',
        }));
    }
    */
    ...
    //             ...result,
    //             no: index + 1,
    //             pinjaman: processedPinjaman // Include the processed pinjaman array
    //         };
    //     });
    // }
    ...
    get processedInvestasiData() {
        return this.investasiData.map(item => ({
            ...item,
            customStatus: item.status === 'Aktif' ? 'status-aktif' : 'status-nonaktif',
        }));
    }
    */
    ...
    // get processedInvestasiData() {
    //     return this.investasiData.map((result, index) => {
    //         // const simpananArray = result?.data?.portofolioPerbankan?.simpanan || [];
    //         const investasiArray = result?.data?.[0]?.portofolioPerbankan?.investasi || [];
    
    //         // Process each investasi
    ...
    //             ...result,
    //             no: index + 1,
    //             investasi: processedInvestasi // Include the processed pinjaman array
    //         };
    //     });
    // }
    ...
    handleClickMutasiSimpanan(event) {
        // Get the data index from the button's data attribute
        console.log('clicked..')
        ...
        // Find the corresponding data based on the index
        console.log('simpananData in handleClickMutasiSimpanan : ' , JSON.stringify(this.simpananData));
        const selectedData = this.simpananData.find(data => data.no === parseInt(index, 10));
        // const selectedData = this.simpananData.find(data => data.accountNumber);
        console.log('selectedData : ' , selectedData);
        
        if (selectedData) {
            // console.log('Selected Data:', selectedData);
            this.selectedNomorRekening = selectedData.accountNumber;
            console.log('Nomor Rekening : ', selectedData.accountNumber);
        } else {
            console.error('Data not found for index:', index);
        }

        // this.isMutasiComponentVisible = true;
    ...
    */
    ...
    handleClickMutasiKredit(event) {
        ...
        // Find the corresponding data based on the index
        const selectedData = this.kreditData.find(data => data.no === parseInt(index, 10));
        
        if (selectedData) {
            // console.log('Selected Data:', selectedData);
            this.selectedNomorKartu = selectedData.nomorKartu;
            console.log('Nomor Kartu : ', selectedData.nomorKartu);
        } else {
            console.error('Data not found for index:', index);
        }
    ...
    */
    ...
    //             setTimeout(() => {
    //                 const scrollableSimpanan = this.template.querySelector('.scrollable-simpanan-container');
    //                 if (scrollableSimpanan) {
    //                     const target = scrollableSimpanan.querySelector('.mutasi-simpanan-component');
    //                     if (target) {
