...
export default class Scc_PenilaianQA_Home extends NavigationMixin(LightningElement) {
    ...
    setFieldVisibility() {
        if (!this.selectedValue || !this.fieldVisibilityConfig.has(this.selectedValue)) {
            console.warn('Config belum ada atau selectedValue belum valid:', this.selectedValue);
            return;
        }

        this.resetFieldVisibility();

        const config = this.fieldVisibilityConfig.get(this.selectedValue);
        console.log(`Config for ${this.selectedValue}:`, config);
