/** 
    ...
    release 3
    [POST DEPLOYMENT R3]
    1.1   20/03/2025   Rakeyan Nuramria                  [FROM SIT] Adjust logic for fetch data API getPortofolio based on the condition.
**/
...
export default class LwcCustomerVerification extends LightningElement {
    ...
    //             console.warn('No AccountId found in the Case record, skipping API call for Customer Verification');
    //         }
    //     } else if (error) {
    ...
    wiredCase({ error, data }) {
        if (data) {
            ...
            console.error('Error fetching Case record:', error);
            this.errorMsgAcc = 'Terjadi kesalahan saat memuat data Case.';
        }
    }
    ...
    wiredAccount({ error, data }) {
        if (data) {
            ...
            if (this.cifNo) {
                ...
                console.warn('CIF not found in the Account record. Skipping API call for Customer Verification');
            }
        } else if (error) {
            console.error('Error fetching Account record:', error);
            this.errorMsgAcc = 'Terjadi kesalahan saat memuat data Account.';
        }
    }
    ...
    connectedCallback() {
        ...
        // this.fetchDataCustomer(); //[20/03/2025 - RKYN] Post Deployment r3, commented because move to the wire method
