({
    ...
    approveSelectedTickets: function(component) {
        ...
        if (selectedCases.length > 0) {
            var action = component.get("c.retryTickets");
            action.setParams({
            ...
            action.setCallback(this, function(response) {
                var state = response.getState();
                if (state === "SUCCESS") {
                    this.showToast("Success", "Retry Tickets successfully", "success");
                    this.fetchCases(component); 
                } else {
                    console.error(response.getError());
                }
            ...
            action.setCallback(this, function(response) {
                ...
                if (state === "SUCCESS") {
                    ...
                    result.data.forEach(function(caseItem) {
                        ...
                        if ((searchNotaValue === 'true' && caseItem.Count_Nota__c > 0) ||
                            (searchNotaValue === 'false' && caseItem.Count_Nota__c === 0) ||
                            searchNotaValue === '') {
