({
    ...
    initialCase : function(component){
        ...
        action1.setCallback(this, function(response1) {
            ...
            if(state1 === "SUCCESS") {
                ...
                // console.log('Unit Kerja Lucas', cs.Unit_Kerja_2__c);
                  
               

                // if (cs.Unit_Kerja_2__c !== undefined) {                    
                //     component.set("v.Unit_Kerja", '6966996');
                // }
                // else{
                //     component.set("v.Unit_Kerja",cs.Unit_Kerja_2__c);
                // }
                ...
                if (cs.Unit_Kerja_2__c === undefined) {
                    console.log('Unit_Kerja_2__c',cs.Unit_Kerja_2__c);
                }else{
                    var action7 = component.get("c.getUnitKerjaDetail");
                    action7.setParams({
                        "unitKerjaId": cs.Unit_Kerja_2__c   
                    });
                    action7.setCallback(this, function(response7) {
                        var state7 = response7.getState();
                        //console.log('Fitur',response7.getReturnValue());
                        if(state7 === "SUCCESS") {
                            component.set("v.Unit_Kerja", response7.getReturnValue());
                        }
                        else{
                            component.set("v.filrec","");
                        }
                ...
                if(cs.SCC_Call_Type__c === undefined){
                    ...
                    console.log('case type : ' , cs.SCC_Call_Type__c);
                    console.log('Case type is not null');
                    var action8 = component.get('c.queryCurrentCaseType8410');
                    action8.setParams({
                        "caseId" : csid
                    });
                    ...
                    action8.setCallback(this,function(response8){
                        ...
                        console.log('state  8410: ', state);
                        
                        
                        if(state === 'SUCCESS'){
                            var result = response8.getReturnValue();
                            console.log('result 8410 :' , result);
                            component.set('v.isCase8410',result);         
                        } else {
                            component.set('v.isCase8410',false);
                ...
                if(cs.SCC_Call_Type__c === undefined){
                    ...
                    console.log('case type : ' , cs.SCC_Call_Type__c);
                    console.log('Case type is not null');
                    var action9 = component.get('c.queryCurrentCaseType8412');
                    action9.setParams({
                        "caseId" : csid
                    });

                    action9.setCallback(this,function(response8){
                        var state = response8.getState()
                        console.log('state  8412: ', state);
                        
                        
                        if(state === 'SUCCESS'){
                            var result = response8.getReturnValue();
                            console.log('result 8412 :' , result);
                            component.set('v.isCase8412',result);         
                        } else {
                            component.set('v.isCase8412',false);
                ...
                /*if(cs.SCC_Call_Type__c != null){
                    console.log('case type not null');
                    var action8 = component.get('c.queryCurrentCaseType8410');
                    action2.setParams({
                        'caseId' : csid
                    });
                    ...
                    action8.setCallback(this, function(response8){
                        ...
                        if(state8 ==='SUCCESS'){
                            var result = response8.getReturnValue();
                            console.log('init case type 8410 : ' , result);
                            
                        } else {
                            console.log('8410 not success');
                        }
                    });
                    $A.enqueueAction(action8)
                }*/
    ...
    getSrcAdjustment : function(component) {
        var csid = component.get("v.recordId");
        console.log('recordId Brizzi Adjustment =',csid);
        var action = component.get("c.getListAdjustment");
        action.setParams({
            "idcs": csid
        });
        action.setCallback(this, function(response) {
            ...
            console.log(state);
            console.log('Response Adjustment Deposit = ',oRes[0]);
            if (state === "SUCCESS"){
                component.set("v.adjustmentColumns",oRes[0]);
                var ad = component.get("v.adjustmentColumns");
                console.log("adjustmentColumns = ",ad);
            }
            else{
                var errors = response.getError();
                this.parseErrorMsg(errors);
    ...
    getSrcReplace : function(component) {
        var csid = component.get("v.recordId");
        console.log('recordId Brizzi Replace =',csid);
        var action = component.get("c.getListReplace");
        action.setParams({
            "idcs": csid
        });
        action.setCallback(this, function(response) {
            ...
            var oRes = response.getReturnValue();
            console.log(state);
            console.log('Response Replace Card = ',oRes[0]);
            if (state === "SUCCESS"){
                component.set("v.replacecolumns",oRes[0]);
                var ad = component.get("v.replacecolumns");
                console.log("replacecolumns = ",ad);
            }
            else{
                var errors = response.getError();
                this.parseErrorMsg(errors);
    ...
    searchRekon : function(component,event){    
        var cs = component.get("v.case");
        var action;
        if(cs.SCC_Case_Type_for_Suggested_Article__c == '8812' || cs.SCC_Case_Type_for_Suggested_Article__c == '8915'){
            console.log("get rekon 8812 or 8815")
            action = component.get("c.getRekon");
        }else if(cs.SCC_Case_Type_for_Suggested_Article__c == '8412'){
            console.log('action rekon edc');
            action = component.get('c.getNewRekonEdc');
        }else {
            console.log('action rekon baru');
            action = component.get("c.getNewRekon");
        }

        // if(cs.SCC_Case_Type_for_Suggested_Article__c != '8812' && cs.SCC_Case_Type_for_Suggested_Article__c != '8915' && cs.SCC_Case_Type_for_Suggested_Article__c != '8412'){
        //     console.log('action rekon baru');
        //     action = component.get("c.getNewRekon");
        // }else {
        //     console.log('action rekon edc');
        ...
        var fiturId = component.get("v.fituridObj")
        ...
        if((cs.SCC_Case_Type_for_Suggested_Article__c == '8812' || cs.SCC_Case_Type_for_Suggested_Article__c == '8915') && tid.value === undefined){
            component.set("v.loaded", true);
            console.log("akhir",component.get("v.loaded"));
            toastEvent4.setParams({
                "title": "Error",
        ...
        else if((cs.SCC_Case_Type_for_Suggested_Article__c != '8713' && (cs.SCC_Case_Type_for_Suggested_Article__c != '8812' || cs.SCC_Case_Type_for_Suggested_Article__c != '8915')) && fiturId.value === undefined){
            component.set("v.loaded", true);
            console.log("akhir",component.get("v.loaded"));
            toastEvent4.setParams({
                "title": "Error",
                "message": "Fitur id tidak boleh kosong",
                "type" : "error"
            });
            toastEvent4.fire();
        }
            ...
            if(fiturId){
                 var fiturIds = fiturId.label.split('-')[0].trim();
            }
           ...
           if(cs.SCC_Case_Type_for_Suggested_Article__c == '8812' || cs.SCC_Case_Type_for_Suggested_Article__c == '8915'){
            ...
            action.setCallback(this, function(response) {
                ...
                console.log(state);
                console.log(response.getReturnValue());
                console.log('fitur id : ' ,fiturId);
                if (state === "SUCCESS"){
                    var oRes = response.getReturnValue();
                    console.log("oRes Rekon: ",oRes);
                    if(oRes.length > 0){
                        ...
                        console.log("PaginationLst Rekon: ",PaginationLst);
            ...
            action.setCallback(this, function(response) {
                ...
                if (state === "SUCCESS"){
                    ...
                    if(oRes.responseDesc == 'success'){
                        component.set('v.DataKartu', oRes); 
                        console.log('Data = ',oRes.data[0]);
                        console.log('Data cardnum = ',oRes.data[0].cardnum);
                        component.set('v.isButtonHistory', false); 
                        component.set('v.isButtonPartnerHistory', false); 
                        var toastEvent = $A.get("e.force:showToast");
                        toastEvent.setParams({
                            "title": "Success!",
                            "message": "Success.",
    ...
    searchTrxHistory : function(component, event){
        var action = component.get("c.getTrxHistory");
        var cs = component.get("v.case");
        var ct = component.get("v.calltypeObj");
        var currentPage = component.get("v.currentPageTrxHistory");
        var pageSize = 10; // Your page size
        var offset = (currentPage - 1) * pageSize; // Calculate the correct offset
        ...
        if(ct.value === undefined) {
            ...
            action.setCallback(this, function(response) {
                ...
                if (state === "SUCCESS"){
                    var oRes = response.getReturnValue();
                    console.log('response trx = ',oRes);
                    if(oRes.responseDesc == 'success'){
                        component.set('v.listTrxHistory', oRes.data);
                        component.set('v.DataTrxHistory', oRes.data);
                        // component.set('v.countTrxHistory', oRes.data.length);
                        if(oRes.data.length >= 10){
                            component.set('v.countTrxHistory', false);
                            console.log('countTrxHistory = ',component.get('v.countTrxHistory'));
                            
                        }
                        else{
                            component.set('v.countTrxHistory', true);
                        }
                        var toastEvent = $A.get("e.force:showToast");
                        toastEvent.setParams({
                            "title": "Success!",
                        ...
                        // If no data returned and we're not on page 1,
                        // we've gone too far - go back one page
                        if(currentPage > 1) {
                            component.set("v.currentPageTrxHistory", currentPage - 1);
                        }
                        
                        // component.set("v.DataTrxHistory", []);
                        component.set('v.countTrxHistory', true);
                        console.log('countTrxHistory = ',component.get('v.countTrxHistory'));
                        var toastEvent2 = $A.get("e.force:showToast");
                        toastEvent2.setParams({
    ...
    searchPtrTrxHistory : function(component, event){
        var action = component.get("c.getPtrTrxHistory");
        var cs = component.get("v.case");
        var ct = component.get("v.calltypeObj");
        var currentPage = component.get("v.currentPagePtrTrxHistory");
        var pageSize = 10; // Your page size
        var offset = (currentPage - 1) * pageSize; // Calculate the correct offset
        ...
        if(ct.value === undefined) {
            ...
            action.setCallback(this, function(response) {
                ...
                if (state === "SUCCESS"){
                    ...
                    console.log('response trx = ',oRes);
                    if(oRes.responseDesc == 'success'){
                        component.set('v.listPtrTrxHistory', oRes.data);
                        component.set('v.DataPtrTrxHistory', oRes.data);
                        if(oRes.data.length >= 10){
                            component.set('v.countPtrTrxHistory', false);
                            console.log('countPtrTrxHistory = ',component.get('v.countPtrTrxHistory'));
                        }
                        else{
                            component.set('v.countPtrTrxHistory', true);
                        }
                        ...
                        // we've gone too far - go back one page
                        if(currentPage > 1) {
                            component.set("v.currentPagePtrTrxHistory", currentPage - 1);
                        }
                ...
                if (state === "SUCCESS") {
                    ...
                    if (oRes && oRes.length > 0) {
                        ...
                        if (code === 200) {
                            ...
                            component.set("v.dateRequest", dateRequest);
                            console.log("dateRequest :",dateRequest);
                            component.set("v.buttonType","NFS");
    ...
    searchEJK: function(component,event){
        var action = component.get("c.getEJLog");
        var cs = component.get("v.case");
        console.log("masuk sini");
        var tidInput = component.find("tidej");  // Get the component with aura:id="tidej"
                ...
                if (state === "SUCCESS") {
                    ...
                    if (oRes && oRes.length > 0) {
                        ...
                        if (code === 200) {
                            ...
                            component.set("v.originalEJTotal", totalLength);
                            console.log('PaginationEJ',totalRecordsList);
                            component.set("v.buttonType","Kaseya");
                            component.set("v.dateRequest", dateRequest);
                            // Reset selected EJ and show success toast
                            component.set("v.selectedItems", []);
                            component.set("v.selectedEJ", 0);
    ...
    getChannelTopup : function(component) {
        // Call server-side method to get picklist values
        var action = component.get("c.getChannelTopupPicklistValues");
        
        action.setCallback(this, function(response) {
            var state = response.getState();
            console.log('getChannelTopup state = ' + state);
            
            if (state === "SUCCESS") {
                var picklistValues = response.getReturnValue();
                console.log('getChannelTopup picklistValues = ' + JSON.stringify(picklistValues));
                
                try {
                    // Ensure picklistValues is an array of objects with label and value
                    ...
                    component.set("v.channelTopupOptions", options);
                    
                    // Verify the options
                    var verifiedOptions = component.get("v.channelTopupOptions");
                    console.log('Verified channelTopupOptions = ' + JSON.stringify(verifiedOptions));
                } catch (e) {
                    console.error('Error processing picklist values: ' + e.message);
                }
            } else {
                var errors = response.getError();
                console.error("Error fetching picklist values: " + JSON.stringify(errors));
            }
        });
