({
    ...
    init: function (cmp, event, helper) {
        ...
        helper.getSrcAdjustment(cmp);
        helper.getSrcReplace(cmp);
        helper.initialCase(cmp);
        // helper.getChannelTopup(cmp);
    ...
    checkboxRekon: function(component, event, helper) {
        ...
        if (selectedRec == true) {
            ...
            console.log('all record else temp' , allRecords)
            console.log('case type : ',cs.SCC_Case_Type_for_Suggested_Article__c);
            for (var i = 0; i < allRecords.length; i++) {
                ...
                if(cs.SCC_Case_Type_for_Suggested_Article__c != '8812' && cs.SCC_Case_Type_for_Suggested_Article__c != '8915'){
                    ...
                    if (recc.data.StatusPenyelesaian != 'Tidak Dikerjakan') {
                        recc.isDisabled = true;
                    }
                }
                // selectedRecords.push(recc);
                selectedRecords.push(recc);
            }
            console.log('vakue selectedRecords : ' , selectedRecords);
            for (var i = 0; i < PaginationList.length; i++) {
                ...
                if(cs.SCC_Case_Type_for_Suggested_Article__c != '8812' || cs.SCC_Case_Type_for_Suggested_Article__c != '8915'){
                    ...
                    if (PaginationList[i].data.StatusPenyelesaian != 'Tidak Dikerjakan') {
                        PaginationList[i].isDisabled = true;
                    }
                }
                updatedPaginationList.push(PaginationList[i]);
        ...
        console.log("value listrekon",selectedRecords);
        
        console.log("value getSelectedNumber",getSelectedNumber);
    ...
    setRekon: function(component, event, helper) {
        ...
        // if(cs.SCC_Case_Type_for_Suggested_Article__c != '8812' &&  cs.SCC_Case_Type_for_Suggested_Article__c != '8915'){
        //     console.log("insert new Rekon");
        //     action = component.get("c.insertNewRekon");
        // }
        // console.log("insert rekon lama ");
        var action;
        if(cs.SCC_Case_Type_for_Suggested_Article__c == '8812' || cs.SCC_Case_Type_for_Suggested_Article__c == '8915'){
            console.log("insert rekon 8812 or 8815")
            action = component.get("c.insertRekon");
        }else if(cs.SCC_Case_Type_for_Suggested_Article__c == '8412'){
            console.log('insert rekon edc');
            action = component.get('c.insertNewRekonEdc');
        }else {
            console.log('insert new baru');
            action = component.get("c.insertNewRekon");
        }
        ...
        if(x>1){
            ...
            if (state === "SUCCESS"){
                if(cs.SCC_Case_Type_for_Suggested_Article__c != '8812' &&  cs.SCC_Case_Type_for_Suggested_Article__c != '8915'){
                    ...
                    console.log('RESPONSE_MESSAGE',res.RESPONSE_MESSAGE);
                    ...
                    if(res.RESPONSE_CODE === '00'){
                        ...
                        var resmsg = '- { koneksi ke rekon gagal }';
                        // var resmsg = '{'+res.ERROR_CODE+' '+res.RESPONSE_MESSAGE+'} - {koneksi ke rekon gagal}';
                        toastEvent.setParams({
                            "title": "Failed!",
                            "message": String(res.ERROR_CODE) + ' ' + String(res.RESPONSE_MESSAGE) + ' ' + String(resmsg),
                            "type" : "error"
                        });
                        toastEvent.fire();
                    }
    ...
    // handleChangeBrizzi: function(component, event, helper) {
    //     var selectedValue = event.getParam("value");
    //     // Update the case field value
    //     component.set("v.case.Channel_Topup__c", selectedValue);
    // },
    ...
    setCase: function(component, event, helper) {
        ...
        var unitKerja = component.get("v.Unit_Kerja");
        // Get the selected Channel Topup value
        // var channelTopup = component.get("v.case.Channel_Topup__c");
        // console.log("value channelTopup" , channelTopup);
        component.set("v.loaded", false);
        console.log("awal",component.get("v.loaded"));
        ...
        if (unitKerja == '') {
            ...
            cs.Unit_Kerja_2__c = unitKerja.value; 
        }
        // if (!channelTopup) {
        //     cs.Channel_Topup__c = '';
        // } else {
        //     cs.Channel_Topup__c = channelTopup;
        // }
        cs.SCC_Status_Transaksi__c = wanotif;
        // console.log("Case channelTopup" , cs.Channel_Topup__c);
    ...
    searchTrxHistory: function (component, event, helper) {
        component.set("v.loaded", false);
        console.log("awal",component.get("v.loaded"));
        helper.searchTrxHistory(component, event);
    },

    searchPtrTrxHistory: function (component, event, helper) {
        component.set("v.loaded", false);
        console.log("awal",component.get("v.loaded"));
        helper.searchPtrTrxHistory(component, event);
    },

    navigationTrxHistory : function(component, event, helper) {
        ...
        component.set("v.currentPageTrxHistory", currentPage);
        
        // Call helper method to fetch new data with updated offset
        helper.searchTrxHistory(component, event);
    },

    navigationPtrTrxHistory : function(component, event, helper) {
        var direction = event.getSource().get("v.name");
        ...
        component.set("v.currentPagePtrTrxHistory", currentPage);
    ...
    searchEJK: function(component, event, helper) {
        component.set("v.loaded", false);
        console.log("awal",component.get("v.loaded"));
        helper.searchEJK(component, event);
    },
    ...
    checkboxEJ: function(component, event, helper) {
        // Ambil seluruh daftar data dari attribute.
        console.log("⚙️ Fungsi checkboxEJ dijalankan...");
        var allItems = component.get("v.PaginationEJ");
        var selectedCount = 0;

        // Looping melalui setiap item untuk menghitung ulang yang dicentang.
        ...
        // Setel nilai akhir ke attribute. Tampilan akan diperbarui secara otomatis.
        component.set("v.selectedEJ", selectedCount);
    },

    setEJ: function(component, event, helper) {
        component.set("v.isProcessing", true);
        var allRecords = component.get("v.listEJ");
        var dateRequest = component.get("v.dateRequest");
        var buttonType = component.get("v.buttonType");
        ...
        for (var i = 0; i < allRecords.length; i++) {
            if (allRecords[i].isChecked) {
                var recordToAdd = allRecords[i].data;
                recordToAdd.num = allRecords[i].num;
                selectedRecords.push(recordToAdd);
            }
        }
    ...
    selectAdjustDeposit: function(component, event, helper) {
        var records = component.get("v.adjustmentColumns");
        console.log('records',records);
        var selectedItems = component.get("v.adjustmentColumns").filter(item => item.IsDeleted__c);
        component.set("v.noSelectionAdjust", selectedItems.length === 0);
    },

    deletedAdjustDeposit: function(component, event, helper) {
        var records = component.get("v.adjustmentColumns");
        var action = component.get("c.deleteAdjustDeposit");
        component.set("v.loaded", false);
        console.log("awal",component.get("v.loaded"));
        action.setParams({
            "listrek" : records 
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS"){
                // Reset the selection state
                component.set("v.noSelectionAdjust", true);
                helper.getSrcAdjustment(component);
                var toastEvent = $A.get("e.force:showToast");
                toastEvent.setParams({
                    "title": "Success!",
                    "message": "Success.",
    ...
    selectReplaceCard: function(component, event, helper) {
        var records = component.get("v.replacecolumns");
        console.log('records',records);
        var selectedItems = component.get("v.replacecolumns").filter(item => item.IsDeleted__c);
        component.set("v.noSelectionReplace", selectedItems.length === 0);
    },

    deletedReplaceCard: function(component, event, helper) {
        var records = component.get("v.replacecolumns");
        console.log("records", records);
        var action = component.get("c.deleteReplaceCard");
        component.set("v.loaded", false);
        console.log("awal",component.get("v.loaded"));
        action.setParams({
            "listrek" : records 
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS"){
                // Reset the selection state
                component.set("v.noSelectionReplace", true);
                helper.getSrcReplace(component);
                var toastEvent = $A.get("e.force:showToast");
                toastEvent.setParams({
                    "title": "Success!",
                    "message": "Success.",
    ...
    doInit: function (component, event, helper) {
        var action = component.get("c.queryCurrentPermissionSet");
        action.setCallback(this, function (response) {
            var state = response.getState();
            if (state === "SUCCESS") {
                var permissionSets = response.getReturnValue();
                //var permissionSets = result.permissionSets || []; 
                //var callType = result.callType || '';             
                //var callType = response.getReturnValue();              
                component.set("v.isSCC_BO_NFT_ISSUER", permissionSets.includes('SCC_BO_NFT_ISSUER'));
                component.set("v.isSCC_BO_Supervisor_NFT_ISSUER", permissionSets.includes('SCC_BO_Supervisor_NFT_ISSUER'));
                // if (callType === '8410') {
                //     component.set("v.isCall_Type__c", true);
                // } else {
                //     component.set("v.isCall_Type__c", false);
                // }
            } else if (state === "ERROR") {
    ...
    CaseType8410: function (component, event, helper) {
        var action = component.get("c.queryCurrentCaseType8410");
        action.setParams({
            caseId: component.get("v.recordId")
        });
        action.setCallback(this, function (response) {
            var state = response.getState();
            console.log('state 8410', state.getReturnValue());
            
            if (state === "SUCCESS") {
                var result = response.getReturnValue();
                console.log('case type : ' , result); 
                component.set("v.isCase8410", result.includes('8410'));
            } else if (state === "ERROR") {
                console.error("Error in fetching:", response.getError());
            }
        });
